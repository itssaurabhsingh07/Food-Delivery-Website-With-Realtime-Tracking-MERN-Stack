        ...
        <div className='w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-[#fff9f6] overflow-visible'>
            ...
            {showsearch && userData.role == "user" && <div className='w-[90%] h-[70px] bg-white shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[5%] md:hidden'>
                ...
                <div className='w-[80%] flex items-center gap-[10px]'>
                    ...
                    <input type='text' placeholder='Search Delicious Items...' className='px-[10px] text-gray-700 outline-0 w-full' />
                </div>
            </div>}
            ...
            <h1 className='text-3xl font-bold mb-2 text-[#ff4d2d]'>WHITmets</h1>
            ...
            {/* Making A div For Mobile there as <> </> */}
            <div className='flex items-center gap-4'>
                {userData.role == "user" && (showsearch ? <RxCross2 size={25}
                    className='text-[#ff4d2d] md:hidden cursor-pointer'
                    onClick={() => setshowsearch(false)} /> :
                    <IoIosSearch size={25} className='text-[#ff4d2d] md:hidden cursor-pointer'
                ...
                {userData.role == "owner" ? <>
                    ...
                    <div className='hidden md:flex items-center p-2 cursor-pointer relative px-3 py-1 rounded-lg bg-[#ff4d2d]/10 text-[#ff4d2d] font-medium' onClick={()=>navigate("/my-orders")}>
                    ...
                    <div className='md:hidden flex items-center p-2 cursor-pointer relative px-3 py-1 rounded-lg bg-[#ff4d2d]/10 text-[#ff4d2d] font-medium' onClick={()=>navigate("/my-orders")}>
                        <TbReceiptRupee size={20} />
                        <span className='absolute -right-2 -top-2 text-xs font-bold text-white bg-[#ff4d2d] rounded-full px-[6px] py-[1px]'>0</span>
                    </div>
                </> : (
                    ...
                    {userData.role=="user" && <div className='relative cursor-pointer' onClick={()=>navigate("/cart")}>
                            ...
                            <span className='absolute right-[-9px] top-[-12px]'>{cartItems.length}</span>
                        </div> }
                ...
                <div className='w-[40px] h-[40px] rounded-full flex items-center justify-center bg-[#ff4d2d] text-white text-[22px] shadow-xl font-semiboldd cursor-pointer ' onClick={() => setshowInfo(prev => !prev)}>
                    {userData?.fullName.slice(0, 1)}
                </div>
                {showInfo && <div className={`fixed top-[80px] right-[10px] ${userData.role=="deliveryBoy" ? "md:right-[20%] lg:right-[40%] ":"md:right-[10%] lg:right-[30%] "}w-[180px] bg-white
                shadow-2xl rounded-xl p-[20px] flex flex-col gap-[10px] z-[999]`}>
                    <div className='cursor-pointer'>{userData.fullName}</div>
