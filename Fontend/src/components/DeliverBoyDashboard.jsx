    ...
    <div className='w-screen min-h-screen flex flex-col gap-5 items-center bg-[#fff9f6] overflow-y-auto'>
      ...
      <div className='w-full max-w-[800px] flex flex-col gap-5 items-center'>
        ...
        {!currentOrder && <div className='bg-white rounded-2xl p-5 shadow-md w-[90%] border border-orange-100'>
          <h1 className='text-lg font-bold flex items-center gap-2'>Available Orders</h1>
              ...
              availableAssignments.map((a, index) => (
                <div className='border rounded-lg p-4 flex justify-between items-center' key={index}>
                  <div>
                    ...
                    <p className='text-sx text-gray-400'>{a?.items?.length} items | {a?.subtotal}</p>
                  </div>
        ...
        {currentOrder && <div className='bg-white rounded-2xl p-5 shadow-md w-[90%] border border-orange-100'>
          ...
          <div className='border rounded-lg p-4 mb-3'>
            ...
            <p className='text-sx text-gray-400'>{currentOrder.shopOrder.shopOrderItems.length} items | {currentOrder.shopOrder.subtotal}</p>
          </div>
          ...
          {!showOtpBox ? <button className='mt-4 w-full bg-green-500 text-white font-semibold py-2 px-4 rounded-xl shadow-md hover:bg-green-600 active:scale-95 transition-all duration-200' onClick={handleSendOtp}>
            Mark As Delivered
          </button>:<div className='mt-4 p-4 border rounded-xl bg-gray-50'><p >Enter Otp Sent to <span className='text-orange-500'>{currentOrder?.user?.fullName}</span></p>
          <input type="text" className='w-full border px-3 py-2 rounded-lg mb-3 focus:outline-none focus:ring-2 focus:ring-orange-400 cursor-pointer' placeholder='Enter Otp'/>
